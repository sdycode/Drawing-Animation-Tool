...
**Now:** ✅ **M7 COMPLETE — autosave hardening + the narrow perf pass.** The only real data-risk in v1 is closed: edits are **debounced** into one write after they settle (AC-10.3.1), the chrome shows a live **dirty / saving / saved / error** indicator (AC-10.3.3), a **failed write retains the edit in memory** and recovers on the next write — no modal, no lost work (AC-10.3.4) — and `rev` advances by exactly 1 per persisted save via a dedicated `_persistedRev` (AC-10.3.5). Teardown is covered by an `AppLifecycleListener` + `onDispose` flush, and Firestore offline persistence queues offline edits (AC-10.3.2). E13: a 114×10 budget document scrubs and drags with **Δ0 panel rebuilds**. Audited by **6 lenses — 3 returned NO FINDINGS** (rev monotonicity, lifecycle/timer discipline, indicator/ephemeral); every real finding clustered on the teardown flush and was fixed (uncommitted-drag leak + unhandled-error escape in `_disposeFlush`, plus an on-Object assert, a strengthened AC-13.1 test, and a stale docstring). Gate: anim_core 452 · anim_render 66 · app 240 · 0 skipped · analyze clean · boundaries ok · web build ✓. Clears `v3/00` §5 criterion #7 (**7 of 9**). **Next: M8** — `anim_core` publish, versioned `.json` export, legacy import over the 8 fixtures, sample gallery (criteria #8/#9). F12.1 deploy stays parked (web build CI-verified).
