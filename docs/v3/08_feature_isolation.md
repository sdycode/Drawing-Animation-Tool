...
| Decode degrades, never validates-and-throws: `as num?` casts, `?? default`, `unknownKeys` on every type, unknown node → `UnknownNode`, orphan pose → dropped + warning | A field feature A adds on Monday makes the document unloadable in feature B on Tuesday |
